...
const head = pair => pair((x,y) => x);  // complete lambda expression
const tail = pair => pair((x,y) => y);  // complete lambda expression
...
const increment_repeater = repeater => f => x => f(repeater,
() => repeater(f)(x));
...
const decrement_repeater = repeater => f => x => repeater((a,b) => a(f)(x))(x);
